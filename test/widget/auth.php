...
if(!empty($_REQUEST['User_Name']) && !empty($_REQUEST['Password']) && !empty($_REQUEST['Imei_No']) && !empty($_REQUEST['Role']))
{
    ...
    if($conn)
    {
		$query = "select Full_Name,User_Name,Password,smtp,Role,User_ID,Company,ControlerNo,Location_Id from user_master where 	User_Name='".$username."' and Password ='".$password."' and smtp= '".$imei_no."' and Role='".$role."'";
		...
		if($row)
		{
			...
			$code = 1;
			echo json_encode(array("response"=>$status,"username"=>$username,"userid"=>$userID,"Role"=>$role, "customerid"=>$customerID, "companyid"=> $companyId ,"controllerNumber" => $controllerNumber,"Location_Id"=> $locationId));
		}
		else
		{
